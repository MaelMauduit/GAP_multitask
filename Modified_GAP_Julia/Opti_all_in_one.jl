...
function get_jitter(h::HyperParams, K_0, Y, train)
    nS = length(h.ϱ)
    vect_one = ones(nS)
    ne, nf, nv = BlockSizes(Y, train)
    Me = Statistics.mean(diag(K_0[1:ne, 1:ne]))
    Mf = Statistics.mean(diag(K_0[ne+1:ne+nf, ne+1:ne+nf]))
    Mv = Statistics.mean(diag(K_0[ne+nf+1:ne+nf+nv, ne+nf+1:ne+nf+nv]))
    ηe = 1e-9 * Me
    ηf = 1e-5* Mf
    ηv = 1e-5* Mv 
...
function full_matrix(h::HyperParams, C::CovMatrices, Y, train)
...
function nll(lh, C::CovMatrices, y, nS, Y, train)
    σ²e = 10^lh[1]
    if nS == 0
        σ² = []
        ϱ = []
...
function g!(G, lh, C::CovMatrices, y, nS, train)
    ...
    h   = HyperParams(σ²e, σ², ϱ)  # ← ordre correct (σ² avant ϱ !)


    Cho = try
        full_matrix(h, C, Y, train)
    catch
...
function optim(C::CovMatrices, y, nS, train)
    w = 2*nS + 1
    f(lh) = nll(vcat(lh), C, y, nS, Y, train)
    G!(G, lh) = g!(G, vcat(lh), C, y, nS, train)
...
function Global_optimizer(X, Y, train, number_of_tasks, Ce, Cf, Cv, Cfe, Cve, Cvf, normalisation = false) 
    ...
    result = optim(C, y, number_of_tasks, train)
    ...
    K1 = hyperparameters(C.Ce, C.Cf, C.Cv, C.Cfe, C.Cve, C.Cvf, σ², ϱ, η, true)
    Me = Statistics.mean(diag(K1[1:ne, 1:ne]))
    Mf = Statistics.mean(diag(K1[ne+1:ne+nf, ne+1:ne+nf]))
    Mv = Statistics.mean(diag(K1[ne+nf+1:ne+nf+nv, ne+nf+1:ne+nf+nv]))
    ηe = 1e-9 * Me
    ηf = 1e-6* Mf
    ηv = 1e-6* Mv 

    σ², ϱ, η = construct_hyperparam(vcat(res, [log10(ηe), log10(ηf), log10(ηv)]), number_of_tasks)
    return σ², ϱ, η
...
function check_gradient_ad(C, y, nS, lh)

    # analytique
    G = zeros(eltype(lh), length(lh))
    @time g!(G, lh, C, y, nS, train)

    # autodiff
    f(x) = nll(x, C, y, nS, Y, train)
...
function plot_nll(X, Y, train, nS, lh0, studied_hyper, low, up,
    ...
    nll_values = [nll(p, C, y, nS, train) for p in lh0_values]
